...
class JetSubCalc(Module):
  ...
  def analyze(self, event):
    ...
    for i,jet in enumerate(Sel_jets):
      ...
      _isTagged, _isTagged_bSFup, _isTagged_bSFdn, _isTagged_lSFup, _isTagged_lSFdn = self.isJetTagged(Sel_jetsP4[i].Pt(),Sel_jetsP4[i].Phi(), jet.btagDeepB, jet.hadronFlavour, i, SFandUncert)
    ...
    for i,jetak8 in enumerate(Sel_jetAK8s):
      ...
      if Sel_SubjetSize > 0 :
        for isubjet in range(SDSubJetIndex, SDSubJetIndex+SDSubjetSize):
          ...
          _isTagged, _isTagged_bSFup, _isTagged_bSFdn, _isTagged_lSFup, _isTagged_lSFdn = self.isJetTagged(subjet.pt,subjet.phi, subjet.btagDeepB, theJetAK8SDSubjetHFlav[isubjet] , isubjet, subjet_SFandUncert) # subjet.hadronFlavour missing, replaced by 0
          if _isTagged :
            nSDSubsDeepCSVMSF +=1
          if _isTagged_bSFup:
  ...
  def isJetTagged(self, pt, phi, btagDeepB, hadronFlavour, Index, SFandUncert):
    ...
    if self.isMC:
      ...
      rand_ = ROOT.TRandom3()
      rand_.SetSeed(int(abs(math.sin(phi)*1e5)))
