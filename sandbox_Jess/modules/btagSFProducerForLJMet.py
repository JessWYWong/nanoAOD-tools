...
class btagSFProducerForLJMet(Module):
    """Calculate btagging scale factors
        algo has to be either 'csvv2' or 'cmva'
    """
    def __init__(self, era, algo , btagOP, sfFileName = None, verbose = 0):
    ...
    def getFlavorBTV(self, flavor):
        ...
        #     flavor_btv = 2
            ...
            flavor_btv = 0   # set default as B unless flavor is C
            # if self.verbose > 0:
    ...
    def analyze(self, event):
        ...
        preloaded_jets = []
        _lightSf_preloaded_jets = []
        for i,jet in enumerate(Sel_jets):
            preloaded_jets.append( (Sel_jetsP4[i].Pt(), Sel_jetsP4[i].Eta(), self.getFlavorBTV(jet.hadronFlavour), getattr(jet, discr)) )
            _lightSf_preloaded_jets.append( (Sel_jetsP4[i].Pt(), Sel_jetsP4[i].Eta(), 2, getattr(jet, discr)) )

        preloaded_subjets = []
        _lightSf_preloaded_subjets = []
        for i, subjet in enumerate(Sel_subjet):
            preloaded_subjets.append( (subjet.pt, subjet.eta, 0, getattr(subjet, discr)) ) # hadron flavor of subjet needed!!!!
            _lightSf_preloaded_subjets.append( (subjet.pt, subjet.eta, 2, getattr(subjet, discr)) )

        # preloaded_jets = [(jet.pt, jet.eta, self.getFlavorBTV(jet.hadronFlavour), getattr(jet, discr)) for jet in jets]

        wp_translation = { "LOOSE" : "L", "MEDIUM" : "M", "TIGHT" : "T"}
        reader = self.getReader( wp_translation.get(self.btagOP, None), False)
        ...
        for central_or_syst in self.central_and_systs:
            ...
            SFandUncert["heavySf_"+central_or_syst] = scale_factors
            _light_scale_factors = list(self.getSFs(_lightSf_preloaded_jets, central_or_syst, reader, 'auto', False))
            SFandUncert["lightSf_"+central_or_syst] = _light_scale_factors
            scale_factors_subjets = list(self.getSFs(preloaded_subjets, central_or_syst, reader, 'auto', False))
            subjet_SFandUncert["heavySf_"+central_or_syst] = scale_factors_subjets
            _light_scale_factors_subjets = list(self.getSFs(_lightSf_preloaded_subjets, central_or_syst, reader, 'auto', False))
            subjet_SFandUncert["lightSf_"+central_or_syst] = _light_scale_factors_subjets

        event.SFandUncert = SFandUncert
        event.subjet_SFandUncert = subjet_SFandUncert
