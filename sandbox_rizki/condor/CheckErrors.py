...
for folder in folders:
	...
	for file in files:
		...
		try:
			...
			good = False
			for line in current:
				if 'PostProcessor took' in line: good = True
			if not good: 
				if verbose_level > 0: 
					print '\tPostProcessor fail:',file,' and JobIndex:',index
				empty_log+=1
		...
		try:
			if not os.path.isfile(dir+'/'+folder+'/'+file.replace('.jdl','.out')): 
				if verbose_level > 0: 
					print '\tNO OUT:',file,' and JobIndex:',index
				no_log += 1
...
print 'NO OUT:', no_log
print 'PostProcessor fail:', empty_log
